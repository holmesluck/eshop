/*firstCateGorygoodslist 用于 firstCateGorygoodslist 页面的事件，
 * 该页面主要是通过主页点击一级目录列表进入的列表页面
 * by张羊左
 */ 
...
if(state1==undefined)
 ...
 //按销量排序的按钮的事件操作
   $("#sales").bind("click",function(){
	       ...
	       var result1=getParameter("firstCatalogId"); 
	       ...
	       $("#sales").attr("href","enterFiCatalogGoods.action?firstCatalogId="+result1+"&priceSort=no&pageNum=0&soldSort="+state);
	        return true;
   }); 
//价格排序的按钮的时间操作
   $("#price").bind("click",function(){
	   ...
	   $("#price").attr("href","enterFiCatalogGoods.action?firstCatalogId="+result1+"&priceSort="+state1+"&pageNum=0&soldSort=no");
       return true;
   });
//快速搜索页面的更多事件按钮
   $("#zyz-more2").bind("click",function(){
	   ...
	   pageNum++;
	$.post("enterFiCatalogGoods.action",{		
		firstCatalogId:result1,
	          priceSort:priceSort,
	          soldSort:soldSort,
	          pageNum:pageNum
          },  function(response){
